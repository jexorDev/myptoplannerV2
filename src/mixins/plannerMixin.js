import {
  getFlexDaysFullScheduleType,
  getFlexDaysHalfScheduleType
} from "@/functions/flexDayCalculator";
...
export default {
  ...
  computed: {
    ...
    flexDays() {
      return this.$store.getters.selectedPlan.participatesInFlex
        ? this.$store.getters.selectedPlan.flexScheduleType === "full"
          ? getFlexDaysFullScheduleType(
              this.$store.getters.selectedPlan.flexDayReferenceDate,
    ...
    totalPtoPlanned() {
      return this.$store.getters.ptoDates.reduce((acc, val) => {
