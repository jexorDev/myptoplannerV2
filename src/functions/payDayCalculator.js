...
export function getPayDays(year, hireDate) {
  ...
  for (let month = 0; month < 12; month++) {
    const midPayDay = getNextBusinessDay(
      moment({ month: month, date: 15, year: year }),
      "previous"
    );
    payDays.push(midPayDay.format("YYYY-MM-DD"));

    const monthEndPayDay = getNextBusinessDay(
      moment({ month: month, date: 1, year: year }).endOf("month"),
      "previous"
    );
