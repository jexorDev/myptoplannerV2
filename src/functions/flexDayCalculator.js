...
export function getFlexDaysFullScheduleType(flexDayReferenceDate, planYear) {
    ...
    const currentDateReverse = moment(flexDayReferenceDate).year(planYearInt).add(-2, 'weeks');
    const currentDateForward = moment(flexDayReferenceDate).year(planYearInt).add(2, 'weeks');
...
export function getFlexDaysHalfScheduleType(planYear) {
    ...
    for (let week = 0; week < 52; week++) {
        flexDays.push(moment().year(planYearInt).week(week).day(5).format("YYYY-MM-DD"));
    }
