...
export function getHolidays(planYear) {
    ...
    holidays.push({description: "Memorial Day", date: memorialDay.format("YYYY-MM-DD")})   

    const independenceDay = getNextBusinessDay(moment(`${planYear}-07-04`), 'next');
    holidays.push({ description: "Independence Day", date: independenceDay.format("YYYY-MM-DD") });

    const laborDay = getNextBusinessDay(moment(`${planYear}-09-01`), 'next', 1);
    holidays.push({ description: "Labor Day", date: laborDay.format("YYYY-MM-DD") });

    const thanksgivingDay = getNextBusinessDay(moment(`${planYear}-11-01`).endOf('month'), 'previous', 4);
    holidays.push({ description: "Thanksgiving", date: thanksgivingDay.format("YYYY-MM-DD") });
    holidays.push({ description: "Thanksgiving", date: thanksgivingDay.add(1, 'day').format("YYYY-MM-DD") });

    const christmasEve = getNextBusinessDay(moment(`${planYear}-12-25`).add(-1, 'day'), 'previous');
    holidays.push({ description: "Christmas Eve", date: christmasEve.format("YYYY-MM-DD") });
