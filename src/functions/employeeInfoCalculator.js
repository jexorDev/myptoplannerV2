...
export function getPtoBreakdown(hireDate, planYear) {
  ...
  const ptoIncreaseDate = moment({
    month: ptoIncreaseMonth,
    date: 1,
    year: planYearInt
  });

  const inYearPriorAnniversary = getEmployeeInYear(
    momentHireDate,
    moment({
      month: momentHireDate.month(),
      date: momentHireDate.date(),
      year: planYearInt
    }).subtract(1, "days")
  );
  const inYearAfterAnniversary = getEmployeeInYear(
    momentHireDate,
    ptoIncreaseDate
  );
  ...
  const totalPtoHoursPriorAnniversary =
    2 * ptoIncreaseDate.month() * ptoHoursPerPayPriorAnniversary;
  const totalPtoHoursAfterAnniversary =
    2 * (12 - ptoIncreaseDate.month()) * ptoHoursPerPayAfterAnniversary;
  ...
  if (moment().year() === momentHireDate.year()) {
    ...
    if (momentHireDate.month() < 12) {
      ptoAccrualHours +=
        2 *
        (12 - (momentHireDate.month() + 1)) *
        ptoHoursPerPayPriorAnniversary;
    }
    ...
    ptoHoursPerPayPriorAnniversary === ptoHoursPerPayAfterAnniversary
  ) {
    ...
    breakdown.push({
      ...
      ptoHoursPerPay: ptoHoursPerPayPriorAnniversary,
      totalPtoHours:
        totalPtoHoursPriorAnniversary + totalPtoHoursAfterAnniversary
    });
  } else {
    breakdown.push({
      dateStart: moment({ month: 0, date: 1, year: planYearInt }),
...
export function getPtoHoursOnDay(date, hireDate, planYear) {
  ...
  if (ptoBreakdown.breakdown.length === 1) {
    ...
    if (
      moment(date).isSameOrBefore(moment(ptoBreakdown.breakdown[0].dateEnd))
    ) {
      return ptoBreakdown.breakdown[0].ptoHoursPerPay;
    } else {
